...
4. **必要になったら権限を段階的に拡張**（読み取り/検索 → 編集 → Git 操作）
...
> 任意のパス（例: `~/GitHub/dotfiles`）にも対応。`setup.sh` 実行時に `~/.dotfiles-root` へ実体パスを記録し、各フック / スクリプト / zshrc がそこから解決します。
...
| `~/.claude/skills/<name>/` | `claude/skills/<name>/`（各スキルを個別シンボリックリンク） |
| `~/.claude/logs/` | （ディレクトリ作成） |
| `~/.local/bin/claude-scripts` | `scripts/` |
| `~/.local/bin/claude-run` | `scripts/claude-run.sh` |
| `~/.zshrc` | `zsh/.zshrc` |
| `~/.gitignore_global` | `git/.gitignore_global` |
| `~/.dotfiles-root` | （テキストファイル: dotfiles 実体パス） |
...
| 手順 | ツール | 内容 |
|---|---|---|
| 前処理 | `nightly-preprocess.py` | stale dates 修正・メトリクス・growth-log ひな型作成 |
| シェル検証 | `nightly-validate-shell.py` | bash -n + shellcheck |
| 後処理 | `nightly-postprocess.py` | growth-log プレースホルダー埋め |
| Master Brain 同期 | `nlm source add`（ローカルのみ） | growth-log エントリを NotebookLM へ送信 |
...
GitHub リポジトリのシークレットに `ANTHROPIC_API_KEY` を設定してください。
