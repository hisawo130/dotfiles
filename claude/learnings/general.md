...
- [javascript] stopImmediatePropagation required (not stopPropagation) when jQuery delegated + document-level listeners coexist — seen 20+ times
- [javascript] Web Component disconnectedCallback must clear all timers and listeners; AbortController simplifies bulk cleanup — seen 15+ times
- [javascript] async scroll side effects (instantJump) must be separated from return values to prevent scrollend race conditions — seen 12+ times
- [javascript] High-frequency event handlers (pointermove): cache getComputedStyle/offsetWidth at drag-start, reuse cached value — seen 12+ times
- [javascript] transitionend: use generation counter to detect and skip stale callbacks from superseded animations — seen 10+ times
- [shopify] Shopify page.content modal HTML is invisible to Liquid templates; verify data-remodal-id via browser DevTools — seen 15+ times
- [shopify] Shopify app deploy: client_id in toml must match an app in the currently-logged-in Partner organization — seen 9+ times
- [shopify] webpack/Sass build failure can delete entire CSS output file; restore via git show before adding new styles — seen 10+ times
- [shopify] feature branch required for Shopify theme work; never commit directly to main — seen 5+ times
...
- [shopify] Matrixify: Email alone can identify customers without ID; use for metafield bulk imports — seen 4+ times
- [email] HTML縦方向余白em統一/横方向px — seen 5 times
- [email] テーブルよりp要素推奨（メールクライアント互換性） — seen 4 times
...
- [pattern] メルマガテンプレートの複数キャンペーン構成：新規キャンペーン（KV・CAMPAIGNボックス・PRESENTセクション）→ 前月引き継ぎセクション → 既存セクション（mnb・SNS・フッター）の順で階層化
- [gotcha] メルマガHTML作成時は実装前に必要な画像ファイル一覧を明記し、未配置ファイルを明確にしておくことでミス防止
- [tip] 複雑なメール構成は番号付けセクション分けで説明し、後続検証・修正時の混乱を回避
- [pattern] メール HTML の商品プレゼント表示は交互レイアウト（画像左/テキスト右、テキスト左/画像右）で縦並びにすると視覚的に整理される
- [gotcha] メール HTML では複数列テーブルより`<p>`タグやシンプルな縦並びが推奨（メールクライアント互換性・スマートフォン対応）
- [tip] スクリーンショット指摘（画像配置、期間表示形式、特典表示）を最初に完全に解析してからコーディング開始で修正回数削減
